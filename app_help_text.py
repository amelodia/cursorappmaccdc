...
La pagina ha due sotto-schede: «Nuova registrazione» (singola immissione) e «Registrazioni periodiche» (elenchi e creazione di registrazioni ricorrenti gestite dal programma). Passa dall’una all’altra con i pulsanti in alto nella stessa area. Nell’elenco delle periodiche, clic sulle intestazioni «Prossima creaz.», «Categoria» o «Conto» ordina le righe (di default per data di prossima esecuzione; un secondo clic inverte l’ordine).
...
SCHEDA «VERIFICA»
Confronto tra movimenti registrati ed estratto conto in PDF per il conto scelto. Avvio sessione, immissione manuale o automatica delle voci, chiusura con salvataggi previsti. Non uscire dalla scheda Verifica con una sessione ancora aperta: usa «Chiudi verifica» (o l’equivalente) prima di cambiare pagina. Se non restano dati in sospeso e il saldo della verifica è zero, la chiusura è confermata senza ulteriore richiesta e si passa a Movimenti; se il conto verificato è una carta di credito, si apre invece Nuove registrazioni con una Girata conto/conto già predisposta (importo negativo pari al saldo dell’estratto, dal conto bancario di riferimento al conto carta, data al primo del mese successivo alla chiusura).
